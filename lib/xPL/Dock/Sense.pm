package xPL::Dock::Sense;

=head1 NAME

xPL::Dock::Sense - xPL::Dock plugin for a Silabs Thunderboard Sense

=head1 SYNOPSIS

  use xPL::Dock qw/Sense/;
  my $xpl = xPL::Dock->new();
  $xpl->main_loop();
...
sub init {
  ...
  $self->{_mqtt} = Net::MQTT::Simple->new('192.168.1.30');
...
sub device_reader {
  ...
  $self->sense_construct_xpl($new_msg);
  #}
  ...
  my @msgs;
	...
	foreach my $line (split /\n/ ,$data) {
    	if ($line =~/(\w+):\s+(\d+\.?\d?)/) {
			...
			$self->{_mqtt}->publish( 'weather/' . $sensor => $value);
    	}
